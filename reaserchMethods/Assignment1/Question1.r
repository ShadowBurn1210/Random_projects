...
# Create proper data frame with predictor variable (Ads, not Customers)
new_data <- data.frame("data$Ads" = seq(100, 800, length.out = 10))

# Generate predictions with the correct new data
predictions <- predict(reg.data,newdata = data.frame(new_data), interval = "predict", level = 0.95)
...
# Add prediction interval lines - using proper indexing
lines(new_data, predictions[, 2], col = "blue", lty = 2)
lines(new_data, predictions[, 3], col = "blue", lty = 2)

# Add a shaded prediction interval area (optional enhancement)
x_polygon <- c(new_data[,1], rev(new_data[,1]))
y_polygon <- c(predictions[,"lwr"], rev(predictions[,"upr"]))
polygon(x_polygon, y_polygon, col = rgb(0, 0, 1, 0.2), border = NA)
...
# Try to check for heteroscedasticity if lmtest is available
has_lmtest <- FALSE
# Check if lmtest package is installed
if(require("lmtest", quietly = TRUE)) {
  ...
  cat("Note: lmtest package not available. Install with install.packages('lmtest') to check for heteroscedasticity.\n")
}
...
# Only include heteroscedasticity findings if lmtest is available
if(has_lmtest) {
  ...
  cat("5. Heteroscedasticity test was not conducted due to missing lmtest package.\n")
}